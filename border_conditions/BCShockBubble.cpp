 ...
 *
 * Implement inlet border conditions according to
 * https://amroc.sourceforge.net/examples/euler/2d/html/shbubble_n.htm
 */
  ...
  if (orchard_key_t<dim>::is_at_any_domain_border(key_inside, m_brick_size))
  {
      ...
      if (m_bc_shock_bubble.needs_override(x, y))
      {
        for (int32_t ivar = 0; ivar < m_userdata.num_vars(); ++ivar)
        {
          m_userdata(cellindex_out, ivar, i_oct_outside) =
            m_bc_shock_bubble(xyz_cell[IX], xyz_cell[IY], ivar);
        }
      }
    }
    else if constexpr (dim == 3)
    {
      ...
      if (m_bc_shock_bubble.needs_override(x, y, z))
      {
        ...
        {
          m_userdata(cellindex_out, ivar, i_oct_outside) =
            m_bc_shock_bubble(xyz_cell[IX], xyz_cell[IY], xyz_cell[IZ], ivar);
        }
      }
    }
